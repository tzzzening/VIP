...
class WasteModel(Model):
    ...
    def __init__(self, seller_num, buyer_num, width, height) -> None:
        ...
        self.schedule.add(buyer)
        # seller = Seller(unique_id=self.next_id(), min_price=5, production_capacity=5, model=self)
        # self.schedule.add(seller)
        # buyer = Buyer(unique_id=self.next_id(), waste_treatment_capacity=25, max_price=5,
        # production_capacity=5, model=self)
        # self.schedule.add(buyer)
